...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Data dari Server:", data); // LIHAT DI CONSOLE F12
      ...
      if (res.ok) {
        ...
        // SESUAIKAN DI SINI:
        if (role === 'INDUSTRI' || role === 'INDUSTRY') {
          router.push('/proyek');
      ...
      alert("Gagal terhubung ke server. Pastikan backend di port 5000 menyala.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-white md:bg-gray-50 p-0 md:p-6 text-black">
      <div className="bg-white w-full max-w-5xl md:rounded-[40px] shadow-2xl shadow-blue-200/20 border border-gray-100 overflow-hidden flex flex-col md:flex-row min-h-[600px] animate-in fade-in zoom-in duration-700">
        ...
        <div className="hidden md:flex md:w-1/2 bg-[#232d42] p-12 flex-col justify-between relative overflow-hidden">
          <div className="relative z-10">
            ...
            <p className="text-gray-400 font-medium">Platform kolaborasi antara Industri dan Sekolah.</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              ...
              <input required type="email" placeholder="nama@email.com"
                className="w-full bg-gray-50 border border-gray-200 p-4 rounded-2xl outline-none focus:ring-4 focus:ring-[#3a57e8]/10 focus:border-[#3a57e8] font-bold text-sm transition-all text-black"
            ...
            <button type="submit" disabled={isLoading}
              className={`w-full bg-[#232d42] text-white py-4 rounded-2xl font-black text-sm shadow-xl shadow-blue-900/10 hover:bg-[#3a57e8] active:scale-[0.98] transition-all uppercase tracking-widest mt-4 ${isLoading ? 'opacity-70' : ''}`}
            >
              {isLoading ? 'Processing...' : 'Masuk Sekarang'}
            </button>

            <div className="pt-8 border-t border-gray-50 text-center">
              <span className="text-gray-400 text-[10px] font-black uppercase tracking-widest mr-2">Belum Punya Akun?</span>
              <Link href="/register" className="text-[#3a57e8] font-black text-[10px] hover:underline uppercase tracking-widest">Daftar Akun Baru</Link>
            </div>
          </form>
        </div>
      </div>
